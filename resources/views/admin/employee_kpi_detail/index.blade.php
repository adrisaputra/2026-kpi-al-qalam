...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#myForm').submit(function (e) {
            ...
            var id_employee_kpi = $('#id_employee_kpi').val();
            ...
            formData.append('id', id_employee_kpi);
            ...
            var url = "{{ url('/employee_kpi_detail/validate') }}";
            $.ajax({
                url: url + "/" + action,
                type: "POST",
                data: formData,
    ...
    function send() {
        ...
        $.ajax({
            url: "{{ url('employee_kpi_detail/store') }}",
            type: "POST",
            data: formData,
    ...
    function getData(id){
        ...
        var url = "{{ url('/employee_kpi_detail/edit') }}";
        $.ajax({
            url: url + "/" + id,
            type: "GET",
            success: function (response) {
                document.getElementById("id_employee_kpi").value = response.data.id;
                document.getElementById("employee_id").value = response.data.employee_id;
                document.getElementById("kpi_category_id").value = response.kpi.kpi_category_id;
                    
                    // Panggil ajax untuk load village berdasarkan subdistrict
                    var url = "{{ url('/kpi/get') }}";
                    $.ajax({
                        url: url + "/" + response.kpi.kpi_category_id,
                        success: function (response2) {
                            $("#kpi_id").html(response2);
                            // Set otomatis terpilih sesuai response.data.village_id
                            $("#kpi_id").val(response.data.kpi_id).trigger("change");
                        }
                    });
            },
            error: function (xhr) {
        ...
        swal({
			...
			if (result.value) {
				swal(
					...
					var url = "{{ url('/employee_kpi_detail/delete') }}";
                    $.ajax({
                        url: url + "/" + id,
                        success: function (response) {
